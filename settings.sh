...
generate_certificate() {
  echo "Installing acme.sh and generating certificate for $domain..."
  curl https://get.acme.sh | sh
  export PATH="$HOME/.acme.sh:$PATH"

  # بدون استفاده از Cloudflare DNS
...
EOF
  ...
  cat >~/free.uri <<-EOF
vless://$uuid@$nsdomain:2096?encryption=none&security=tls&sni=$domain&alpn=http%2F1.1&fp=chrome&type=ws&host=$domain&path=$paths#free-with-proxy-config
EOF
...
echo "Done. Xray installed with WS+TLS on Cloudflare Full SSL."
